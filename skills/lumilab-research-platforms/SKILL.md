...
| `TIKHUB_API_KEY` 缺失 + 无 Chrome profile | **不落 mock** → 走「宿主代搜」(`source: agent-pending`)，你用自己的能力补全（见下节） |
| `TAVILY_API_KEY` 缺失 | **不落 mock** → 走「宿主代搜」(`source: agent-pending`)，你用自己的能力补全（见下节） |
| key 存在但调用失败（401/429/网络） | 同样走宿主代搜兜底，**不退 mock**；并提示用户检查 token |
...
**核心原则：没有外置 API key ≠ 假数据。** 你（宿主 agent）本身知识面很宽、通常带 web 工具——所以缺 key 时把检索交还给你，产出**真实**结果，mock 只在显式 `--mock`（测试）时才出现。

当 `web_tavily.ts` / `xhs_tikhub.ts` 打印 `⟁ AGENT_SEARCH_NEEDED` 或写出的 json `source: "agent-pending"` 时，**按这棵决策树补全，不要接受 mock**：

```
你当前宿主有 web 搜索能力吗（WebSearch / WebFetch / 已装的搜索 skill）？
├─ 有 → 用它逐条真搜 queries[]，每条取前 8~10 条真实结果（带真实 URL+摘要）
│        → --mode web
└─ 没有 → 用你的训练知识写出你「确知为真」的竞品/痛点/市场事实
          （url 可留空，但内容必须是真知识，绝不可编造）
          → --mode knowledge
```

补全后用 ingest 归一回写（schema 与 Tavily/TikHub 完全一致，下游 0 改动）：

```bash
# 1) 把你搜到/答出的结果写成数组 json，例：/tmp/agent_web.json
#    web : [{title,url,text,published_date?,score?}]
#    xhs : [{title,desc,author?,likes?,collects?,comments?,tags?,url?}]
# 2) 归一回写（source 自动标 agent-web / agent-knowledge）
bun run scripts/ingest_agent_results.ts --venture <slug> --channel web --mode web --query "<q>" --in /tmp/agent_web.json
```

守卫：ingest 会**拒绝**空/占位/无 URL（web 模式）输入并 `exit 3`——这是刻意的，逼你补真实内容而非静默落假数据。`source` 取值：`agent-web`（你实时检索）/ `agent-knowledge`（你凭知识作答）/ `tavily`·`tikhub`（真 API）/ `mock`（仅 `--mock` 测试）。
...
| host LLM | 宿主提供 | 取决于宿主 | ~3-8k tokens / synthesis | 综合分析复用宿主；**也是缺 key 时的代搜兜底** |
| Tavily API | Web 搜索 | 免费额度 + 付费 | basic 1 credit / advanced 2 credit（每月 1000 credit 免费额度，超出 ~$0.008/credit） | 缺 key → 宿主代搜（非 mock） |
| TikHub API | XHS 数据 | 付费 | ~$0.01 / 次搜索（按配额计） | 缺 key → 宿主代搜 或 Playwright（非 mock） |
...
`lumilab research-xhs "AI 副业" --venture my-v --limit 10`（有 key 真抓；无 key 自动转宿主代搜，产真实数据）
...
`E_401` 立即停止并提示 token 失效；`E_429` 等待 + 指数退避；**key 失败 / 缺 key → 宿主代搜兜底（`source: agent-pending` → `agent-web`/`agent-knowledge`），不退 mock**。`--mock` 仍保留，仅供测试 / 纯离线 demo。

## Edge cases

各来源（`tavily`/`tikhub`/`agent-web`/`agent-knowledge`/`mock`）都明确标 `source` + `notice`，下游 skill 可识别可信度；所有来源 schema 严格一致，下游 0 改动。`agent-knowledge` 来源无 live URL 属正常（凭知识作答），下游展示时应标注「来自宿主 agent 认知，非实时检索」。
