...
### Fixed
- **构建页「实时 re-theme」右侧落地页不变** —— 根因：面板设的是 canonical 变量（`--accent` 等），但 landing 的 styles.css 读的是 `--color-accent` 等别名，名字对不上。现在实时预览同时桥接 `--color-*` 别名（与 apply 时 theme.css 的桥接一致），拖旋钮右侧 landing 真的实时变。landing-mvp 的 `:root` 也改用 canonical token 名，未来 landing 原生支持实时调样式。
- **小红书图本该走 HTML 渲染却走了 AI 生图**（尤其在 Codex 里）—— SKILL.md 把两条路径并列、"可选"标得太弱。现在加**硬规则**：配图默认且只默认走 `render-xhs-card.ts`（HTML 模板渲染，0 崩中文字、免费），AI `imagegen.ts` 改为**显式 opt-in**（加 `--opt-in` 守卫 + 醒目提示），并写进 Anti-Slop / 必做约束 / 分支决策。
- **小红书「6 张图文案但只出 2 张图」不匹配** —— 规格写 6 图分镜但工具每次只渲染 1 张封面卡。现在规格对齐：默认只出 1 张封面，要轮播则逐张渲染、张数=文案屏数；去掉模板里误导的 "Nº 01 / 03"。

### Changed
- **构建页颜色改用取色器**（`<input type=color>`）替代手输 oklch 数值；oklch→hex 自动转换，preset 也走 hex。
- **方向「用此方向生成 Landing」只给可复制提示词**，不再直接改页面「已选方向」（方向由 agent 实际生成时写回 decisions.yaml）。
- **启动页重排**：网站 / 验证页置顶 → 验证信号（埋点漏斗）+ 付款数据**在此完整展示**（不只在概览）→ 小红书 / 活动内容画廊在下。
- **小红书正文纯文本 + emoji，禁 Markdown**（平台不渲染 `**`/`##`/`-`）—— 写进平台规则 + SKILL.md；Studio 文案框可滚动看全文。

### Removed
- 复盘「脑图」视图（看板/脑图切换）+ 想法澄清假设的「列表/表格/看板」切换 —— 都是无实际功能的占位，去掉。
...
### Changed
- **License：Apache-2.0 → AGPL-3.0**（源码可见 + 强 copyleft）。免费用 / 改 / 自部署；但**改了再分发、或作为网络服务对外提供，必须把完整源码同样以 AGPL-3.0 开放** —— 不能拿去做闭源产品。闭源 / 专有商用另议。26 个 skill frontmatter `license:` + manifest + LICENSE 全量更新；README / README.en 的 License 段重写，致谢补 garrytan/gstack。
